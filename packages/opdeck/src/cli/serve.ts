...
export function registerServe(program: Command): void {
  program
    .command('serve')
    .description(
      'Serve the compiled bundle as an MCP server (stdio by default, or --http for a standalone HTTP transport — the sidecar/container mode). Set OPDECK_TARGET_HEADERS (a JSON object of header name to value, e.g. \'{"authorization":"Bearer ..."}\') to inject credentials into every proxied request. Over --http, set OPDECK_MCP_TOKEN to unlock the surface (unset = every request 404s). Secrets live in the environment only, never in config files.',
    )
