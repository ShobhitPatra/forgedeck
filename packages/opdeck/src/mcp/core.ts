...
export function buildToolHandlers(
  ...
  for (const def of manifest.tools) {
    ...
    // the shared secret in `x-opdeck-bridge-token` (read from OPDECK_BRIDGE_TOKEN
    ...
    if (def.kind === 'server-action') {
      handlers.set(def.name, {
        ...
        async run(args) {
          ...
          const token = process.env.OPDECK_BRIDGE_TOKEN
          const headers: Record<string, string> = {
            ...injected,
            'content-type': 'application/json',
          }
          if (token) headers['x-opdeck-bridge-token'] = token
...
/**
 * Parse the OPDECK_TARGET_HEADERS env var: a JSON object of header name → value.
...
export function parseTargetHeaders(raw: string | undefined): Record<string, string> | undefined {
  ...
  try {
    ...
    console.error(
      `opdeck: OPDECK_TARGET_HEADERS is not valid JSON, ignoring credential headers: ${
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    console.error(
      'opdeck: OPDECK_TARGET_HEADERS must be a JSON object of string headers, ignoring credential headers',
    )
    return undefined
  }
  const headers: Record<string, string> = {}
...
export function buildMcpServer(
  ...
  const server = new McpServer({ name: `opdeck ${manifest.app}`, version: '0.0.1' })
