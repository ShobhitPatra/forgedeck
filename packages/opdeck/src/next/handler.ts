...
// The flagship adapter: `createOpdeckHandler()` turns a Next.js App Router route
...
//
// LOCKED BY DEFAULT (v1 spec D2, normative). Adding opdeck never exposes an app:
//   - OPDECK_MCP_TOKEN unset  -> EVERY request is an empty 404 (an invisible door,
//     not an open one).
...
//
// PUBLIC OPT-OUT. Setting `OPDECK_PUBLIC=0` makes this handler IGNORE any discovered
...
export function resolveBundleDir(
  ...
  throw new Error(
    `opdeck: could not locate the ${dirName} bundle (no tools.json found). Tried:\n` +
      tried.map((p) => `  - ${p}`).join('\n') +
      `\nRun \`opdeck build\` (or \`withOpdeck()\` at build time) and ensure ` +
 ...
 * lives in the same process it drives. `OPDECK_TARGET_URL` overrides for the rare
 * split-deploy case; otherwise we point at the app's own port (`PORT`, default 3000).
 */
export function resolveTargetUrl(): string {
  if (process.env.OPDECK_TARGET_URL) return process.env.OPDECK_TARGET_URL
...
export interface CreateOpdeckHandlerOptions {
...
export type OpdeckRouteHandler = (request: Request) => Promise<Response>
 ...
 *
 * BOTH the Next App Router handler (`createOpdeckHandler`) and the standalone HTTP
...
export async function handleMcpRequest(
  request: Request,
  opts: CreateOpdeckHandlerOptions = {},
): Promise<Response> {
  const makeTransport =
  ...
  const makeServer =
    ...
    ((manifest, targetUrl) =>
      buildMcpServer(manifest, targetUrl, {
        headers: parseTargetHeaders(process.env.OPDECK_TARGET_HEADERS),
  ...
  const token = process.env.OPDECK_MCP_TOKEN
  ...
  if (authorized) {
    ...
    // OPDECK_PUBLIC=0 is the deploy-time opt-out: ignore any discovered
    // `.agent-public/` and keep the route locked (empty 404) for anonymous callers.
    if (process.env.OPDECK_PUBLIC === '0') return notFound()
 ...
 */
export function createOpdeckHandler(opts: CreateOpdeckHandlerOptions = {}): {
  GET: OpdeckRouteHandler
  POST: OpdeckRouteHandler
} {
