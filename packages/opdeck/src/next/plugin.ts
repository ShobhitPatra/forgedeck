/**
 * `withOpdeck()` — the SYNC leg (spec D1, plan addendum P2/P6).
 *
 ...
 *
 * `withOpdeck(nextConfig)` returns an **async config function**
 ...
 * reads by path. So `withOpdeck` merges
 ...
 * over the `opdeck build && next build` escape hatch. This merge is applied on
...
export type OpdeckConfigFn = (phase: string, context?: unknown) => Promise<NextConfig>
 ...
 */
export function mergeOpdeckTracing(config: NextConfig): NextConfig {
...
const GUARD = Symbol.for('opdeck.plugin.compiled')
...
export function __resetOpdeckGuard(): void {
  const g = globalThis as unknown as Record<symbol, Set<string> | undefined>
  g[GUARD] = undefined
}
...
export interface OpdeckPluginDeps {
  /** The SHARED build orchestrator — the same `runBuild` the `opdeck build` CLI
...
const defaultDeps: OpdeckPluginDeps = {
  build: realRunBuild,
...
// plus the two synthesized WARN classes `opdeck coverage` reports (a write with
...
function oneLine(s: BuildStats): string {
  return `opdeck ✓ ${s.actions} actions (${s.readsEnabled} reads enabled, ${s.mutationsLocked} mutations locked) · ${s.entities} entities · ${s.warnings.length} warnings → .agent/`
}
...
function steadyStateVoice(ir: SemanticIR): string {
  ...
  if (s.warnings.length > shown.length) {
    out.push(
      `  … +${s.warnings.length - shown.length} more — run \`opdeck coverage\` for the full report`,
    )
  } else if (s.warnings.length > 0) {
    out.push('  run `opdeck coverage` for the full report')
  }
  return out.join('\n')
}
...
function teachingTable(ir: SemanticIR): string {
  ...
  if (locked.length === 0) {
    ...
    for (const a of locked.slice(0, CAP))
      lines.push(`    - ${a.name} (${a.effect} locked by default — enable in opdeck.config.ts)`)
  ...
  lines.push('')
  lines.push('  next: run `opdeck coverage` for the full authorization report')
  lines.push('  next: enable specific mutations by name in opdeck.config.ts')
  return lines.join('\n')
}
...
function failureBanner(err: unknown): string {
  ...
  return [
    '',
    '⚠ opdeck: agent extraction FAILED — the build continues, last-good .agent/ kept.',
    `  reason: ${reason}`,
    '  the app still deploys; the agent surface is unchanged from the last successful build.',
    '  run `opdeck build` locally to see the full error.',
    '',
  ].join('\n')
}
 ...
 * exactly as they do for `opdeck build`. The plugin keeps only its own concerns:
 ...
 */
export async function runOpdeckExtraction(
  projectDir: string,
  deps: OpdeckPluginDeps = defaultDeps,
): Promise<void> {
 ...
 */
export function withOpdeck(
  nextConfig: NextConfig = {},
  deps: OpdeckPluginDeps = defaultDeps,
): OpdeckConfigFn {
  return async (phase: string): Promise<NextConfig> => {
    const merged = mergeOpdeckTracing(nextConfig)
    ...
    if (isBuildPhase && !isWorker) {
      ...
      if (!alreadyCompiled(projectDir)) {
        await runOpdeckExtraction(projectDir, deps)
      }
    }
    return merged
  }
