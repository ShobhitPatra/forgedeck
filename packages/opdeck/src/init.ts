/**
 * `opdeck init` — the 60-second adoption path (spec addendum P3).
 *
 ...
 * minutes; instead `init` runs OUR fast structural compile (`runOpdeckExtraction`,
 * the same compile+voice code `withOpdeck()` runs inside `next build`). It
...
import { runOpdeckExtraction } from './next/plugin.js'

// The version that first ships `opdeck diff` (Task 5 / the review leg). The
// generated workflow calls `opdeck diff`, which lands before launch; the YAML
...
const SHIM = `import { createOpdeckHandler } from 'opdeck/next'

export const { GET, POST } = createOpdeckHandler()
`
...
const CONFIG_SCAFFOLD = `import { defineConfig } from 'opdeck'

/**
 * opdeck configuration.
 *
 * opdeck is safe by default: every READ action is exposed to agents, and
...
function workflowYaml(): string {
  return `# opdeck semantic diff — comments the agent-surface changes on every PR.
# requires opdeck >= ${DIFF_VERSION} (the release that ships \`opdeck diff\`).
name: opdeck diff
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: 22
      - run: npm install
      - run: npx opdeck diff --base \${{ github.event.pull_request.base.sha }}
`
}
...
const defaultDeps: InitDeps = {
  ...
  firstBuild: (projectDir) => runOpdeckExtraction(projectDir),
}
...
/**
 * Attempt to wrap a config source in `withOpdeck(...)`. Returns the rewritten
...
export function tryWrapConfig(source: string, style: ConfigStyle): string | null {
  ...
  const importLine =
    style === 'esm'
      ? `import { withOpdeck } from 'opdeck/next'`
      : `const { withOpdeck } = require('opdeck/next')`
  return `${importLine}\n${source.slice(0, rhsStart)}withOpdeck(${source.slice(rhsStart, rhsEnd)})${source.slice(rhsEnd)}`
}
...
function pasteLines(fileName: string, style: ConfigStyle): string[] {
  const [imp, wrap] =
    style === 'esm'
      ? [
          `  import { withOpdeck } from 'opdeck/next'`,
          `  export default withOpdeck(/* your existing config */)`,
        ]
      : [
          `  const { withOpdeck } = require('opdeck/next')`,
          `  module.exports = withOpdeck(/* your existing config */)`,
        ]
  return [
    `${fileName}: config shape not recognized — wrap it by hand (opdeck never guesses at your config):`,
    imp,
    wrap,
  ]
}
...
/**
 * Run `opdeck init` against `projectDir`. Idempotent, non-interactive, and it
...
export async function runInit(
  ...
  if (!appBase) {
    log('opdeck init currently supports Next.js App Router projects')
    return
  }
  ...
  if (!configFile) {
    ...
    if (source.includes('withOpdeck')) {
      ...
      if (wrapped) {
        ...
        log(`✓ wrapped ${configFile} with withOpdeck()`)
      } else {
  ...
  const cfgAbs = join(projectDir, 'opdeck.config.ts')
  if (existsSync(cfgAbs)) {
    log('opdeck.config.ts: already set up ✓')
  } else {
    writeFileSync(cfgAbs, CONFIG_SCAFFOLD)
    log('✓ wrote opdeck.config.ts')
  }
  ...
  if (options.withWorkflow) {
    const wfRel = join('.github', 'workflows', 'opdeck.yml')
    ...
    log('hint: run `opdeck init --with-workflow` to comment semantic diffs on every PR')
  }
