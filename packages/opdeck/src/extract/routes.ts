...
// opdeck's own MCP shim (`export const { GET, POST } = createOpdeckHandler()`
// with the import from 'opdeck/next') is infrastructure, not an app action: it
// SERVES the agent surface rather than belonging on it. Without this check the
// extractor warns 'wrapped route handler not resolved: createOpdeckHandler' for
...
function isOpdeckMcpRoute(sf: SourceFile): boolean {
  return sf
    .getImportDeclarations()
    .some(
      (imp) =>
        imp.getModuleSpecifierValue() === 'opdeck/next' &&
        imp.getNamedImports().some((s) => s.getName() === 'createOpdeckHandler'),
    )
}
...
export function extractRoutes(
  ...
  for (const sf of loaded.project.getSourceFiles()) {
    ...
    if (isOpdeckMcpRoute(sf)) {
      skipped.push({ file: rel, reason: 'opdeck mcp route' })
      continue
    }
