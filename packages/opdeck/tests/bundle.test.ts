...
describe('emitBundle', () => {
  const out = mkdtempSync(join(tmpdir(), 'opdeck-'))
  let ir: SemanticIR
  let written: string[]
  beforeAll(async () => {
  ...
  it('index lists counts', () => {
    ...
    expect(idx).toContain('7 actions')
  })
  it('coverage report renders', () => {
    const report = renderCoverage(ir)
    expect(report).toContain('7 actions extracted')
...
describe('emitBundle auth and evidence (hybrid-shop)', () => {
  const out = mkdtempSync(join(tmpdir(), 'opdeck-hs-'))
  let ir: SemanticIR
  beforeAll(async () => {
