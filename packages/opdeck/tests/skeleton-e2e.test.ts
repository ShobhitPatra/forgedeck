...
//      FROZEN), install the packed opdeck tarball into it;
//   2. wrap next.config.js with withOpdeck() (CJS require of the ESM package
...
//   4. `next start` with OPDECK_MCP_TOKEN: the MCP initialize handshake
...
describe.runIf(runE2E)('skeleton e2e: withOpdeck + createOpdeckHandler, deploy-shaped', () => {
  let appDir: string
  ...
  beforeAll(() => {
    ...
    // 2. Pack a LEAN opdeck tarball (a temporary .npmignore keeps tests/ and
    ...
    writeFileSync(
      ...
      `import { createOpdeckHandler } from 'opdeck/next'\n\nexport const { GET, POST } = createOpdeckHandler()\n`,
    )
  ...
  it('prints the build voice exactly once (main process only, never per worker)', () => {
    const voices = buildOutput.split('\n').filter((l) => l.startsWith('opdeck ✓'))
    expect(voices).toHaveLength(1)
    expect(voices[0]).toMatch(/^opdeck ✓ \d+ actions \(\d+ reads enabled, \d+ mutations locked\)/)
  })

  it('emits the bundle and does not warn about its own mcp shim', () => {
    expect(existsSync(join(appDir, '.agent', 'tools.json'))).toBe(true)
    expect(buildOutput).not.toContain('wrapped route handler not resolved: createOpdeckHandler')
  })
  ...
  it(
    ...
    async () => {
      ...
      expect(body.result?.serverInfo?.name).toBe('opdeck mini-shop')
