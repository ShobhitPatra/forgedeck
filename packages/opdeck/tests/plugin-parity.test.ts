...
import { withOpdeck, __resetOpdeckGuard } from '../src/next/plugin'
import { runBuild } from '../src/build'
import { ConfigError } from '../src/config/schema'

// Parity: a build driven by the `withOpdeck()` plugin must produce EXACTLY what
// `opdeck build` (runBuild) produces — same bundle, same bridges, same storefront.
...
describe('plugin build parity with cli build', () => {
  it('a plugin-driven build produces byte-identical bundle, bridges, and storefront to a cli build', async () => {
    ...
    writeFileSync(join(pluginDir, 'opdeck.config.ts'), FULL_CONFIG)
    writeFileSync(join(cliDir, 'opdeck.config.ts'), FULL_CONFIG)

    // CLI leg: exactly what `opdeck build` runs.
    await runBuild(cliDir, join(cliDir, '.agent'))

    // Plugin leg: the real withOpdeck config function on the build phase, keyed off cwd.
    await inDir(pluginDir, async () => {
      __resetOpdeckGuard()
      await withOpdeck({})(PHASE_BUILD)
    })
  ...
  it('a ConfigError fails the build via the plugin (the one deliberate build-breaker)', async () => {
    ...
    await inDir(dir, async () => {
      __resetOpdeckGuard()
      await expect(withOpdeck({})(PHASE_BUILD)).rejects.toThrowError(ConfigError)
      __resetOpdeckGuard()
      // the explanation (why config errors are the one exception) is attached
      await expect(withOpdeck({})(PHASE_BUILD)).rejects.toThrow(ConfigError.explanation)
    })
