...
afterEach(() => {
  if (savedToken === undefined) delete process.env.OPDECK_MCP_TOKEN
  else process.env.OPDECK_MCP_TOKEN = savedToken
})
...
describe('startHttpMcpServer — token posture over a live server', () => {
  ...
  it('404s all requests when OPDECK_MCP_TOKEN is unset (locked by default)', async () => {
