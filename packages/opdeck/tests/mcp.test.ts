...
describe('buildToolHandlers — bridge dispatch for server actions', () => {
  const savedToken = process.env.OPDECK_BRIDGE_TOKEN
  afterEach(() => {
    if (savedToken === undefined) delete process.env.OPDECK_BRIDGE_TOKEN
    else process.env.OPDECK_BRIDGE_TOKEN = savedToken
  })
  ...
  it('dispatches an enabled server action to its bridge URL with the token header and JSON body', async () => {
    process.env.OPDECK_BRIDGE_TOKEN = 'secret-token'
    ...
    expect((init as RequestInit).headers).toMatchObject({
      'x-opdeck-bridge-token': 'secret-token',
      authorization: 'Bearer creds',
      'content-type': 'application/json',
    })
  })

  it('omits the token header when OPDECK_BRIDGE_TOKEN is unset', async () => {
    ...
    const [, init] = fetchMock.mock.calls[0]
    expect((init as RequestInit).headers).not.toHaveProperty('x-opdeck-bridge-token')
  })
