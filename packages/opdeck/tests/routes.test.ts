...
describe('extractRoutes', () => {
  ...
  it('extracts all six route handlers', () => {
...
describe('extractRoutes — the opdeck mcp shim is infrastructure, not an action', () => {
  it('skips the shim quietly (coverage note, no unresolved-wrapper warning)', () => {
    ...
    expect(actions).toEqual([])
    expect(skipped).toEqual([{ file: 'app/api/mcp/route.ts', reason: 'opdeck mcp route' }])
  })
  ...
  it('recognizes the shim by its import, not its path', () => {
    ...
    expect(actions).toEqual([])
    expect(skipped).toEqual([{ file: 'app/api/agent-door/route.ts', reason: 'opdeck mcp route' }])
  })
    ...
    const { actions, skipped } = extractRoutes(
      fakeLoaded({
        '/app/api/things/route.ts': [
          `import { createOpdeckHandler } from './local-helpers'`,
          `export const GET = createOpdeckHandler()`,
        ].join('\n'),
      }),
    )
    ...
    expect(skipped).toEqual([
      {
        ...
        reason: 'wrapped route handler not resolved: createOpdeckHandler',
      },
    ])
  })
})
