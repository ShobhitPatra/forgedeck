...
type RecordingDeps = OpdeckPluginDeps & {
  logs: string[]
  warns: string[]
...
function makeDeps(over: Partial<OpdeckPluginDeps> = {}): RecordingDeps {
  const logs: string[] = []
  const warns: string[] = []
  const counts = { buildCalls: 0 }
  const base: OpdeckPluginDeps = {
    build: async () => {
      counts.buildCalls++
      return result(ir())
    },
...
beforeEach(() => __resetOpdeckGuard())

// --- tracing merge --------------------------------------------------------

describe('mergeOpdeckTracing', () => {
  it('injects /api/mcp -> .agent glob into an empty config', () => {
    const merged = mergeOpdeckTracing({})
  ...
  it('preserves other tracing keys and existing /api/mcp entries (no clobber)', () => {
    ...
    const merged = mergeOpdeckTracing(input)
  ...
  it('is idempotent — does not duplicate the glob if already present', () => {
    const once = mergeOpdeckTracing({})
    const twice = mergeOpdeckTracing(once)
  ...
  it('withOpdeck returns a config carrying the tracing merge on any phase', async () => {
    const fn = withOpdeck({ reactStrictMode: true }, makeDeps())
...
describe('build voice', () => {
  it('prints the exact one-line voice on a steady-state build', async () => {
    ...
    await runOpdeckExtraction('/proj', deps)
    ...
    expect(firstLine).toBe(
      'opdeck ✓ 3 actions (1 reads enabled, 2 mutations locked) · 2 entities · 1 warnings → .agent/',
    )
    ...
    expect(deps.logs[0]).toContain('run `opdeck coverage` for the full report')
  })

  it('echoes up to five warnings then points to opdeck coverage', async () => {
    const many = ir({
      coverage: {
        extracted: 3,
    ...
    expect(out[out.length - 1]).toContain('run `opdeck coverage`')
  ...
  it('prints the fuller teaching table on the first build (no prior .agent/)', async () => {
    ...
    expect(table.startsWith('opdeck ✓ 3 actions')).toBe(true)
    ...
    expect(table).toContain('next: run `opdeck coverage`')
  ...
  it('steady-state build with no warnings prints only the one line', async () => {
    ...
    expect(deps.logs[0]).toBe(
      'opdeck ✓ 1 actions (1 reads enabled, 0 mutations locked) · 2 entities · 0 warnings → .agent/',
    )
...
describe('never break the build', () => {
  it('swallows an extraction failure: loud warn, no emit, config still returned', async () => {
    ...
    const fn = withOpdeck({ reactStrictMode: true }, deps)
    const cfg = await fn(PHASE_BUILD)
