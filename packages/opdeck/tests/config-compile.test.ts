...
// Resolution reads OPDECK_ENV -> NODE_ENV -> base; vitest sets NODE_ENV=test, so
...
// optional opdeck.config.ts, so config wiring is exercised end-to-end through
...
function makeProject(config?: string): string {
  ...
  if (config !== undefined) write('opdeck.config.ts', config)
  return dir
}
...
describe('compile lists unapplied environment blocks', () => {
  it('notes present-but-dormant environment blocks in coverage', async () => {
    ...
    // No OPDECK_ENV -> base is active, staging is present but dormant.
    expect(ir.coverage.skipped).toContainEqual({
      file: 'opdeck.config.ts',
