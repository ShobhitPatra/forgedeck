...
import { handleMcpRequest, type CreateForgedeckHandlerOptions } from './next/handler.js'
...
export interface StartHttpMcpServerOptions {
  ...
  handlerOpts?: CreateForgedeckHandlerOptions
 ...
 * FORGEDECK_MCP_TOKEN unset -> every request is an empty 404; wrong/missing bearer ->
...
export function startHttpMcpServer(opts: StartHttpMcpServerOptions): Promise<HttpMcpServerHandle> {
  ...
  const server = createServer((req, res) => {
    void (async () => {
      try {
        ...
        console.error(`forgedeck: HTTP MCP request failed: ${(err as Error).message}`)
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(opts.port, opts.host, () => {
      ...
      console.error(
        `forgedeck: MCP HTTP server listening on port ${port} (bundle ${opts.bundleDir}, target ${opts.targetUrl})`,
      )
      resolve({
        server,
        port,
