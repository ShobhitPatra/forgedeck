...
// The config is forgedeck's authorization audit surface: one committed, diffable
...
export function defineConfig(config: ForgedeckConfig): ForgedeckConfig {
  return config
}

/**
 * A config error is the sole deliberate exception to forgedeck's never-break-the-build
...
export class ConfigError extends Error {
  static readonly explanation =
    'Config errors fail the build deliberately — the one exception to forgedeck never breaking your build, because shipping with misread authorization config is worse than not shipping.'
 ...
 * Selection: FORGEDECK_ENV -> NODE_ENV -> base. The selected environment's
...
export function resolveEnvironment(
  ...
  env: { FORGEDECK_ENV?: string; NODE_ENV?: string },
): ResolvedEnvironment {
  let name: string
  let source: ResolvedEnvironment['source']
  if (env.FORGEDECK_ENV) {
    name = env.FORGEDECK_ENV
    source = 'FORGEDECK_ENV'
