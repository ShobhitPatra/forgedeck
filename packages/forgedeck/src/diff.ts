/**
 * `diff` orchestration — the library half of `forgedeck diff` (spec D3, P4).
 *
...
export function renderDiffMarkdown(diff: IRDiff): string {
  ...
  const out: string[] = ['### forgedeck semantic diff', '']
...
export async function runDiff(baseRef: string, projectDir: string): Promise<RunDiffResult> {
  ...
  const worktree = mkdtempSync(join(tmpdir(), 'forgedeck-diff-'))
  try {
