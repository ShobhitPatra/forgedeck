/**
 * Lint-level validation of the forgedeck-diff Action + dogfood workflow (Task 6).
 *
...
const actionYml = join(repoRoot, '.github', 'actions', 'forgedeck-diff', 'action.yml')
const workflowYml = join(repoRoot, '.github', 'workflows', 'forgedeck-diff.yml')
...
describe('forgedeck-diff action.yml', () => {
  ...
  it('upserts a sticky comment via the marker using github-script', () => {
    expect(yml).toContain('<!-- forgedeck-diff -->')
...
describe('forgedeck-diff workflow', () => {
  ...
  it('triggers only on fixture changes and grants pull-requests: write', () => {
    ...
    expect(yml).toContain('packages/forgedeck/tests/fixtures/**')
  ...
  it('invokes the local composite action with the PR base sha', () => {
    expect(yml).toContain('./.github/actions/forgedeck-diff')
