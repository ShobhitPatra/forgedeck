...
// compile. The real firstBuild (runForgedeckExtraction) is covered by the CLI
...
describe('forgedeck init', () => {
  ...
  it('bails politely when there is no App Router', async () => {
    ...
    expect(h.logs).toEqual(['forgedeck init currently supports Next.js App Router projects'])
    // Nothing was written.
    expect(existsSync(join(dir, 'forgedeck.config.ts'))).toBe(false)
  ...
  it('writes the 5-line shim (app layout) and ends on the first build', async () => {
    ...
    expect(shim).toContain(`import { createForgedeckHandler } from 'forgedeck/next'`)
    expect(shim).toContain('export const { GET, POST } = createForgedeckHandler()')
  ...
  it('wraps a CJS object-literal config (module.exports = {})', async () => {
    ...
    expect(out).toContain(`const { withForgedeck } = require('forgedeck/next')`)
    expect(out).toContain('module.exports = withForgedeck({})')
    expect(h.logs).toContain('✓ wrapped next.config.js with withForgedeck()')
  })
  ...
  it('wraps an ESM object-literal config (.mjs export default {…})', async () => {
    ...
    expect(out).toContain(`import { withForgedeck } from 'forgedeck/next'`)
    expect(out).toContain('export default withForgedeck({\n  reactStrictMode: true,\n})')
  })
  ...
  it('wraps an identifier config (export default nextConfig)', async () => {
    ...
    expect(out).toContain('export default withForgedeck(nextConfig)')
  })
  ...
  it('never mangles an ambiguous (composed) config — prints the paste-line, leaves it untouched', async () => {
    ...
    expect(h.logs.some((l) => l.includes(`module.exports = withForgedeck(`))).toBe(true)
  })
  ...
  it('never mangles a function config (export default (phase) => …)', async () => {
    ...
    expect(h.logs.some((l) => l.includes('config shape not recognized'))).toBe(true)
    expect(h.logs.some((l) => l.includes('export default withForgedeck('))).toBe(true)
  })
  ...
  // --- forgedeck.config.ts scaffold --------------------------------------

  it('scaffolds an all-comments forgedeck.config.ts', async () => {
    const dir = track(appProject())
    await runInit(dir, {}, harness().deps)
    const cfg = readFileSync(join(dir, 'forgedeck.config.ts'), 'utf8')
    expect(cfg).toContain(`import { defineConfig } from 'forgedeck'`)
  ...
  it('prints only a hint without --with-workflow', async () => {
    ...
    const h = harness()
    await runInit(dir, {}, h.deps)
    expect(existsSync(join(dir, '.github', 'workflows', 'forgedeck.yml'))).toBe(false)
  ...
  it('writes the diff workflow with --with-workflow', async () => {
    ...
    const yml = readFileSync(join(dir, '.github', 'workflows', 'forgedeck.yml'), 'utf8')
    expect(yml).toContain('on:\n  pull_request:')
    expect(yml).toContain('fetch-depth: 0')
    expect(yml).toContain('actions/setup-node@v4')
    expect(yml).toContain('npx forgedeck diff --base ${{ github.event.pull_request.base.sha }}')
    expect(yml).toContain('requires forgedeck >=')
    expect(h.logs).toContain('✓ wrote .github/workflows/forgedeck.yml')
  })
  ...
  it('is idempotent: a second run changes nothing and reports "already set up ✓"', async () => {
    ...
    const paths = [
      ...
      'forgedeck.config.ts',
      '.gitignore',
      '.github/workflows/forgedeck.yml',
    ]
    ...
    for (const marker of [
      ...
      'forgedeck.config.ts: already set up ✓',
      '.gitignore: already set up ✓',
      '.github/workflows/forgedeck.yml: already set up ✓',
    ]) {
...
describe('tryWrapConfig', () => {
  it('wraps bare object literals and identifiers, rejects everything else', () => {
    ...
    expect(tryWrapConfig('export default cfg\n', 'esm')).toContain(
      'export default withForgedeck(cfg)',
    )
    ...
    expect(tryWrapConfig(`export default { a: '}' }\n`, 'esm')).toContain(
      `export default withForgedeck({ a: '}' })`,
    )
  })
})
...
describe('forgedeck init (CLI, real build)', () => {
  const dirs: string[] = []
  afterEach(() => {
    for (const d of dirs.splice(0)) rmSync(d, { recursive: true, force: true })
  })
  ...
  it('runs end-to-end on a mini-shop copy and prints the teaching table', () => {
    ...
    expect(existsSync(join(dir, 'forgedeck.config.ts'))).toBe(true)
    ...
    expect(stdout).toContain('forgedeck ✓')
