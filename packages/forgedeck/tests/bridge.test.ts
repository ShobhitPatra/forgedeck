...
// The bridge runtime reads its shared secret from FORGEDECK_BRIDGE_TOKEN and gates on
// the `x-forgedeck-bridge-token` request header. Save/restore so cases are isolated.
const saved = process.env.FORGEDECK_BRIDGE_TOKEN
afterEach(() => {
  if (saved === undefined) delete process.env.FORGEDECK_BRIDGE_TOKEN
  else process.env.FORGEDECK_BRIDGE_TOKEN = saved
})
...
describe('createBridgeHandler — lock 2 (invisible posture)', () => {
  it('404s with an empty body when FORGEDECK_BRIDGE_TOKEN is unset', async () => {
    delete process.env.FORGEDECK_BRIDGE_TOKEN
    const handler = createBridgeHandler(async () => ({ ok: true }))
    const res = await handler(post({ id: 'x' }, { 'x-forgedeck-bridge-token': 'anything' }))
    expect(res.status).toBe(404)
    expect(await res.text()).toBe('')
  })

  it('404s when the token header is missing', async () => {
    process.env.FORGEDECK_BRIDGE_TOKEN = 'secret'
    const handler = createBridgeHandler(async () => ({ ok: true }))
  ...
  it('404s when the token header is wrong', async () => {
    process.env.FORGEDECK_BRIDGE_TOKEN = 'secret'
    const handler = createBridgeHandler(async () => ({ ok: true }))
    const res = await handler(post({ id: 'x' }, { 'x-forgedeck-bridge-token': 'nope' }))
    expect(res.status).toBe(404)
    expect(await res.text()).toBe('')
  })
})
...
describe('createBridgeHandler — happy path', () => {
  it('invokes fn with the parsed body and returns its result as JSON', async () => {
    ...
    const res = await handler(post({ id: 'abc' }, { 'x-forgedeck-bridge-token': 'secret' }))
  ...
  it('passes undefined when the request body is empty', async () => {
    ...
    const res = await handler(post(undefined, { 'x-forgedeck-bridge-token': 'secret' }))
    expect(res.status).toBe(200)
...
describe('createBridgeHandler — lock 3 (never swallow the app’s own auth)', () => {
  it('propagates errors thrown by fn instead of catching them', async () => {
    ...
    await expect(
      handler(post({ id: 'x' }, { 'x-forgedeck-bridge-token': 'secret' })),
    ).rejects.toThrow('Unauthorized: no session')
  })
})
